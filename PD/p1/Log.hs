...
data MessageType = Info | Warning | Error Int -- ^ Este parametro indica la gravedad del error
  deriving (Show, Eq)
...
data MaybeLogMessage = ValidLM LogMessage | InvalidLM String
  deriving (Show, Eq)
...
testParse :: (String -> [LogMessage]) -> Int -> FilePath -> IO ()
...
--   archivo de log @f@.
testWhatWentWrong :: (String -> [LogMessage]) -> ([LogMessage] -> [String]) -> FilePath -> IO ()
